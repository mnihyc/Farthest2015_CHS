...
from HCommon import SplitParam
...
def UnpackScd(filepath: str, outpath: str, enc: str):
	cs = CStruct()
...
def RepackScd(filepath: str, outpath: str, enc: str):
	with open(filepath, 'r', encoding=enc) as f:
		lines = [line.strip() for line in f.readlines()]
	idt = SplitParam(lines[0])
	name = lines[1].partition('"')[2].rpartition('"')[0]
	first_block = []
	for i in range(_FST_BLK_LINE, len(lines)):
		if lines[i].startswith('FstBlk:'):
			first_block.append(SplitParam(lines[i])[1:])
		else:
			lines = lines[i:]
			break
	cs = CStruct()
...
if __name__=='__main__':
	...
	if cmd == "unpack":
		UnpackScd(infile, outfile, enc)
	elif cmd == "pack":
		RepackScd(infile, outfile, enc)
	else:
